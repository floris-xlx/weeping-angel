...
| Field | Value |
| --- | --- |
| Status | **Phase 0–3 advanced.** Schema SSOT closed (`DEBT-SCHEMA-DUP` / DUP-001). Catalog root walk + applicability.toml parse consolidated (DUP-008 / DUP-013). Lineage applicability types renamed (DUP-004 migrating). DUP-002 `require_needles` copies closed (C01: `tests/support/mod.rs`, inventory `require_needles_fns` = 1). Remaining: full snapshot-shape merge, readiness constructors. |
