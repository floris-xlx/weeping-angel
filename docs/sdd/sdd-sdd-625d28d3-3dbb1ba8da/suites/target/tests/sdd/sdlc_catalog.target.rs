...
//!
//! Encodes DESIRED behavior (AC-1…AC-16). Must stay RED on the current
//! tree: no SDLC family documents, no seven multi-repo fixtures, and no
//! `sdd_sdlc_catalog_*` harness rows. Do not implement catalog content here.
//!
...
//! `EvidenceValue`, or a repository-inventory resolver.
...
fn ac1_dual_suite_is_registered() {
    ...
    assert!(
        text_has(&toml, "sdd_sdlc_catalog_baseline")
            && text_has(&toml, &baseline)
            && text_has(&toml, "sdd_sdlc_catalog_target")
            && text_has(&toml, &target),
        "AC-1: dual-suite sdd_sdlc_catalog_baseline + sdd_sdlc_catalog_target must be listed in root Cargo.toml"
    );
    assert!(
        manifest_dir().join(&baseline).is_file(),
        "AC-1: {baseline} must exist (harness/implement registers it)"
    );
}
...
fn ac2_workspace_gates_keep_sibling_suites() {
    ...
    for (suite, path) in sibling_suite_paths() {
        assert!(
            text_has(&toml, suite) && text_has(&toml, path),
    ...
    for id in CANONICAL_SDLC_CONTROLS {
        ...
        assert!(
            (!control.title.is_empty() && !control.objective.is_empty())
                || text_has(control_record_window(&text, id), "objective")
                || text_has(control_record_window(&text, id), "description"),
...
fn ac6_seven_fixtures_distinguish_evaluator_outcomes() {
    ...
    for name in SDLC_FIXTURES {
        ...
        assert!(
            text_has(&blob, "evidence.repository.")
                ...
                || text_has(&blob, "Exception"),
            "AC-6: fixture `{name}` must emit canonical SDLC facts"
        );
        assert!(
    ...
    assert!(
        excepted_subjects.iter().any(|s| text_has(s, "repo:legacy"))
...
fn ac16_manifest_lists_sdlc_files_and_digest_is_deterministic() {
    ...
    let has_sdlc = ["source", "cicd", "release", "supply-chain", "sdlc"]
        .iter()
        .any(|needle| {
            text_has(&listed, &format!("controls/{needle}"))
                || text_has(&listed, &format!("evidence/{needle}"))
                || text_has(&listed, &format!("tests/{needle}"))
                || text_has(&listed, &format!("{needle}.toml"))
        });
    assert!(
        has_sdlc,
    ...
    assert!(
        text_has(&listed, "fixture.example") && text_has(&listed, "identity"),
...
fn ac9_iso_pack_source_sliver_and_mappings_unchanged() {
    ...
    assert!(
        text_lacks(&mappings, "control.source.default-branch-protection")
...
fn ac11_no_second_loader_value_enum_or_repo_inventory_resolver() {
    ...
    assert!(
        text_has(&rust, "struct CanonicalCatalog")
            && text_has(&rust, "fn load")
            && text_has(&rust, "fn validate"),
    ...
    let forbidden_ty = format!("struct {}", ["Sdlc", "Population"].join(""));
    assert!(
        text_lacks(&rust, &format!("fn {forbidden_fn}")) && text_lacks(&rust, &forbidden_ty),
        "AC-11: do not add a repository-inventory resolver or SdlcPopulation; use inventory.subject + inventory.complete"
    );
    ...
    assert!(
        ...
        ) || text_has(&control_src, "weeping_angel_evidence::EvidenceValue"),
        "AC-11: consume the landed EvidenceValue"
    );
    assert!(
        text_lacks(&control_src, "enum EvidenceValue"),
        "AC-11: do not define a second EvidenceValue enum in control-test"
    );
    assert!(
        text_has(&control_src, "inventory.subject") && text_has(&control_src, "inventory.complete"),
...
fn ac14_prompt01_and_iam_ssot_are_not_overwritten() {
    ...
    assert!(
        text_has(&prompt01, "weeping-angel/canonical-catalog/v1")
            && text_has(&prompt01, "control.source.protected-branch")
            && text_has(&prompt01, "CAT-015"),
        "AC-14: Prompt 01 SSOT still owns catalog infrastructure + CAT fixture"
    );
    assert!(
        text_lacks(&prompt01, "sdd_sdlc_catalog_target")
            && text_lacks(&prompt01, "control.source.default-branch-protection"),
    ...
    assert!(
        text_has(&iam, "control.identity.mfa"),
        "AC-14: IAM SSOT still owns the identity family"
    );
    assert!(
        text_lacks(&iam, "sdd_sdlc_catalog_target")
            && text_lacks(&iam, "control.source.default-branch-protection"),
...
fn ac15_gitlab_or_bitbucket_collector_can_populate_same_contracts() {
        ...
        .chain(quoted_ids(&text, "test."))
    {
        ...
        for token in FORBIDDEN_NATIVE_ID_TOKENS {
            assert!(
                !id_has_token(&id.to_ascii_lowercase(), token),
                "AC-15: GitHub-native object name `{token}` must not appear in canonical id `{id}`"
            );
        }
    }
}
