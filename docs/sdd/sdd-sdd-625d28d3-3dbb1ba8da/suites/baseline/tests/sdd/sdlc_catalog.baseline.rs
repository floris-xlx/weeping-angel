...
//! GitHub-shaped source sliver, IAM family, and Prompt 03 population
//! semantics. Additive Prompt 05 IDs stay compatible — this suite does not
//! require the SDLC family to be absent after it lands. Does not assert
//! harness `[[test]]` registration.
...
fn iso_hybrid_kind_in_toml_loads_as_automated() {
    ...
    assert!(
        ...
        "pack source still marks source branch-protection as hybrid"
    );
...
fn expired_and_revoked_exceptions_do_not_except() {
    ...
    let mut revoked_set = failing.clone();
    revoked_set.insert_exception(bound_exception(ExceptionStatus::Revoked, None, "repo:b"));
    assert_eq!(
        evaluate(&test, &revoked_set, &ctx).effectiveness,
        Effectiveness::Ineffective,
        "Revoked status does not skip the failing subject"
    );

    let mut timed_out = failing.clone();
    timed_out.insert_exception(bound_exception(
        ExceptionStatus::Approved,
        Some(-1),
        "repo:b",
    ));
    assert_eq!(
        evaluate(&test, &timed_out, &ctx).effectiveness,
        Effectiveness::Ineffective,
        "Approved but expires_at <= now does not except"
    );

    let mut unbound = failing;
    ...
    assert_eq!(
        evaluate(&test, &unbound, &ctx).effectiveness,
        Effectiveness::Ineffective,
...
fn manual_control_cannot_auto_pass_without_attestation() {
    ...
    assert_eq!(
        still.effectiveness,
        Effectiveness::ManualReviewRequired,
        "a technical source/release envelope is not a manual attestation"
    );
    ...
    let expr_manual = compiled(
        "test.release.authorization",
        "control.release.authorization",
        TestExpr::ManualReview,
    );
    assert_eq!(
        evaluate(&expr_manual, &attested, &ctx).effectiveness,
        Effectiveness::ManualReviewRequired,
        "TestExpr::ManualReview cannot auto-pass from any envelope"
    );
}
...
fn framework_and_collector_crates_stay_catalog_blind() {
    ...
    let catalog_crate = read_crate_toml("weeping-angel-canonical-catalog");
    assert!(
