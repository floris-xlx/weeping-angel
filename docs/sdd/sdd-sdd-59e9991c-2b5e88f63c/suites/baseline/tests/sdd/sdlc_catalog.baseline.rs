//! Baseline suite for the Prompt 05 SDLC catalog slice (additive / CI-004).
//!
//! Encodes product invariants that are true on snapshot head
//! `e2def07ee4c3ec265a6b5fee116931f0b2c9ce94` and must remain true after
//! the 26-control SDLC family lands. Does **not** assert absence of
//! `control.source.default-branch-protection` (that would be replacement).
//! Does **not** characterize `Cargo.toml` `[[test]]` registration.
...
fn load_catalog() -> CanonicalCatalog {
    CanonicalCatalog::load(catalog_v1()).expect("canonical catalog loads offline")
}
...
fn catalog_loader_validate_and_digest_remain_the_single_ssot() {
    ...
    let second = again.digest().expect("digest");
    assert_eq!(
        first.to_string(),
        second.to_string(),
...
fn fixture_protected_branch_is_exists_only_and_survives() {
    ...
    assert_eq!(
        evidence.evidence_type, "source.branch.protection",
...
fn iso_and_fixture_existence_pass_on_one_envelope() {
    ...
    let iso = exists_test("source.branch.protection");
    ...
    assert_eq!(
        ...
        "today ISO/fixture existence is satisfied by any one source.branch.protection envelope"
    );
...
fn github_collector_still_advertises_source_star_types() {
    ...
    assert!(
        !github.contains("resolve_repository_inventory"),
        "collector has no repository-inventory resolver fork"
    );
}
...
fn no_repository_inventory_resolver_or_second_value_enum() {
    ...
    let identity = pop
        .find("fn resolve_identity_inventory")
        .expect("identity inventory special-case remains");
    let generic = pop
        .find("inventory.subject")
        .expect("generic inventory.subject path remains");
    let complete = pop
        .find("inventory.complete")
        .expect("generic inventory.complete path remains");
    assert!(
        identity < generic && generic < complete || identity > 0 && complete > 0,
        "resolve_population still consumes identity then inventory.subject + inventory.complete"
    );
...
fn population_uses_inventory_subject_and_complete() {
    let ctx = fresh_context();
    let expr = repo_all_subjects("source.branch.protection", "protected");
    let test = compiled(
...
fn evaluator_distinguishes_missing_stale_fail_manual_and_exception() {
    ...
    let manual_r = evaluate(&manual, &EvidenceSet::new(), &ctx);
    assert_eq!(
        manual_r.effectiveness,
        Effectiveness::ManualReviewRequired,
        "ManualReview cannot auto-pass"
    );
    ...
    let mut ex = Exception::new(ExceptionId::new("exc:repo-b"), "timeboxed waiver");
    ex.status = ExceptionStatus::Approved;
    ex.control_id = Some(ControlId::new(PINNED_CONTROL));
    let mut ids = BTreeSet::new();
    ids.insert("repo:b".into());
    ex.subjects
    ...
    excepted.insert_exception(ex);
    let except_r = evaluate(&test, &excepted, &ctx);
    assert_eq!(
        except_r.effectiveness,
        Effectiveness::Effective,
        "approved unexpired subject-scoped exception skips the failing repo"
    );
...
fn expired_and_revoked_exceptions_do_not_except() {
    ...
    let test = compiled(
        "test.source.default-branches-protected",
        PINNED_CONTROL,
        repo_all_subjects("source.branch.protection", "protected"),
    );

    fn bound(status: ExceptionStatus, expires_hours: Option<i64>) -> Exception {
        let mut ex = Exception::new(ExceptionId::new("exc:repo-b"), "waiver");
        ex.status = status;
        ex.expires_at = expires_hours.map(|h| {
            Utc.with_ymd_and_hms(2026, 8, 18, 12, 30, 0).unwrap() + chrono::Duration::hours(h)
        });
        let mut ids = BTreeSet::new();
        ids.insert("repo:b".into());
        ex.subjects
    ...
    authoritative_repos(&mut expired_set, &["repo:a", "repo:b"]);
    ...
    expired_set.insert_exception(bound(ExceptionStatus::Expired, None));
    let expired_r = evaluate(&test, &expired_set, &ctx);
    assert_eq!(
        expired_r.effectiveness,
        Effectiveness::Ineffective,
        "Expired status does not skip the failing subject"
    );

    let mut revoked_set = EvidenceSet::new();
    authoritative_repos(&mut revoked_set, &["repo:a", "repo:b"]);
    revoked_set.insert(seal(
        "source.branch.protection",
        "repo:a",
        &[("protected", "true")],
    ));
    ...
    revoked_set.insert_exception(bound(ExceptionStatus::Revoked, None));
    let revoked_r = evaluate(&test, &revoked_set, &ctx);
    assert_eq!(
        revoked_r.effectiveness,
        Effectiveness::Ineffective,
        "Revoked status does not skip the failing subject"
    );

    let mut timed_out = EvidenceSet::new();
    authoritative_repos(&mut timed_out, &["repo:a", "repo:b"]);
    timed_out.insert(seal(
        "source.branch.protection",
        "repo:a",
        &[("protected", "true")],
    ));
    ...
    timed_out.insert_exception(bound(ExceptionStatus::Approved, Some(-1)));
    let timed_r = evaluate(&test, &timed_out, &ctx);
    assert_eq!(
        timed_r.effectiveness,
        Effectiveness::Ineffective,
        "Approved but expires_at <= now does not except"
    );

    let mut unbound = EvidenceSet::new();
    authoritative_repos(&mut unbound, &["repo:a", "repo:b"]);
    unbound.insert(seal(
        "source.branch.protection",
        "repo:a",
        &[("protected", "true")],
    ...
    let mut empty = Exception::new(ExceptionId::new("exc:unbound"), "no subjects");
    empty.status = ExceptionStatus::Approved;
    unbound.insert_exception(empty);
    let unbound_r = evaluate(&test, &unbound, &ctx);
    assert_eq!(
        unbound_r.effectiveness,
        Effectiveness::Ineffective,
        "empty Exception.subjects does not mean the whole inventory"
    );
}

#[test]
fn manual_control_cannot_auto_pass_without_attestation() {
    let ctx = fresh_context();
    let manual = CompiledControlTest::builder()
        .id(ControlTestId::new(
            "test.source.secure-development-policy-attested",
        ))
        .control_id(ControlId::new("control.source.secure-development-policy"))
    ...
    assert_eq!(
        empty.effectiveness,
        Effectiveness::ManualReviewRequired,
        "Manual kind without attestation cannot auto-pass"
    );

    let mut tech = EvidenceSet::new();
    tech.insert(seal(
        "source.branch.protection",
        "repo:a",
        &[("protected", "true")],
    ));
    let still = evaluate(&manual, &tech, &ctx);
    assert_eq!(
        still.effectiveness,
        Effectiveness::ManualReviewRequired,
        "a technical source envelope is not a manual attestation"
    );
    ...
    let ok = evaluate(&manual, &attested, &ctx);
    assert_eq!(
        ok.effectiveness,
        Effectiveness::Effective,
        "fresh manual_attestation is the only auto-pass path for Manual kind"
    );
}

#[test]
fn framework_and_collector_crates_stay_catalog_blind() {
    let framework = read_crate_toml("weeping-angel-framework");
    for forbidden in [
        ...
        assert!(
            !framework.contains(forbidden),
            "framework Cargo.toml stays catalog-blind (`{forbidden}`)"
        );
    }
    assert!(framework.contains("weeping-angel-assurance-ir"));

    let collector = read_crate_toml("weeping-angel-collector");
    for forbidden in [
        ...
        assert!(
            ...
            "collector Cargo.toml stays framework/catalog-blind (`{forbidden}`)"
        );
    }
...
#[test]
fn scanner_engines_are_not_catalog_contracts() {
    let catalog = catalog_toml_joined();
    for needle in [
        "GITHUB_EVIDENCE_TYPES",
        "src/engines",
        "src/depcheck",
        ...
        assert!(
            ...
            "canonical catalog TOML must not couple to scanner/collector internals (`{needle}`)"
        );
    }
    let _ = ControlDomain::ChangeManagement;
    let _ = ControlDomain::VulnerabilityManagement;
