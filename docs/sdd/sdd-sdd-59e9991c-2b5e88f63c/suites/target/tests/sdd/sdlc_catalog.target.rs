...
//!
//! Encodes DESIRED behavior from `docs/prompts/canonical-assurance-v1/05-sdlc-catalog.md`
//! and ADR `docs/adr/0003-sdlc-canonical-assurance-catalog-draft.md` (AC-1…AC-16).
//! Consumes landed CanonicalCatalog + EvidenceValue + evaluate_coverage.
//! Does not implement catalog content, fork a second loader, or change Prompt 03.
...
const CANONICAL_SDLC_CONTROLS: &[&str] = &[
    ...
    "control.source.repository-visibility",
...
const PROMPT05_EXAMPLE_TESTS: &[&str] = &[
    ...
    "test.source.reviewer-count",
    "test.source.security-scanning-enabled",
    "test.cicd.workflow-permissions-minimized",
    "test.release.protected-environment",
    "test.source.dependency-scanning-current",
    "test.supply-chain.artifact-integrity",
];
...
const FORBIDDEN_FRAMEWORK_TOKENS: &[&str] =
    &["iso27001", "iso-27001", "soc2", "soc-2", "nis2", "nis-2"];
...
fn sdlc_catalog_text() -> String {
    ...
    for path in catalog_toml_files() {
        ...
        if text.lines().any(|line| {
            is_sdlc_id(line)
                || SDLC_ID_PREFIXES.iter().any(|p| line.contains(p))
                ...
                || line.contains("test.source.")
                || line.contains("test.cicd.")
                || line.contains("test.release.")
                || line.contains("test.supply-chain.")
        }) {
            chunks.push(text);
        }
    }
...
fn fixture_dir(name: &str) -> PathBuf {
    manifest_dir()
        .join("fixtures/assurance/canonical/v1/sdlc")
        .join(name)
}
...
fn force_push(set: &mut EvidenceSet, id: &str, restricted: bool) {
    set.insert(seal(
        "evidence.repository.branch-protection",
        id,
        &[
...
fn review_required(set: &mut EvidenceSet, id: &str, required: bool) {
    set.insert(seal(
        "evidence.repository.review-policy",
        id,
        &[
            ("subject_id", id),
            ("review_required", if required { "true" } else { "false" }),
            ("production", "true"),
        ],
        collected(1),
    ));
...
fn dependency_scan(set: &mut EvidenceSet, id: &str, current: bool) {
    set.insert(seal(
        "evidence.repository.dependency-scanning",
        id,
        &[
            ("subject_id", id),
            ("current", if current { "true" } else { "false" }),
        ],
        collected(1),
    ));
}
...
fn workflow_permissions(set: &mut EvidenceSet, id: &str, minimized: bool) {
    set.insert(seal(
        ...
        &[
            ("subject_id", id),
            ("write_minimized", if minimized { "true" } else { "false" }),
        ],
        collected(1),
    ));
}
...
fn provenance(set: &mut EvidenceSet, id: &str, present: bool) {
    set.insert(seal(
        "evidence.supply-chain.build-provenance",
        id,
        &[
...
fn healthy_population() -> EvidenceSet {
    ...
    for id in ["repo:app", "repo:lib", "repo:api"] {
        ...
        force_push(&mut set, id, true);
        review_required(&mut set, id, true);
        security_scan(&mut set, id, true, 1);
        dependency_scan(&mut set, id, true);
        workflow_permissions(&mut set, id, true);
        env_protection(&mut set, id, true);
        provenance(&mut set, id, true);
    }
    set
}
...
#[test]
fn ac2_workspace_gates_and_sibling_suites_remain() {
    let toml = fs::read_to_string(manifest_dir().join("Cargo.toml")).unwrap();
    for suite in [
        "sdd_iso27001_assurance_target",
        "sdd_iam_catalog_target",
        "sdd_canonical_assurance_catalog_target",
        "sdd_sdlc_catalog_target",
        "sdd_sdlc_catalog_baseline",
    ] {
        assert!(
            toml.contains(suite),
            "AC-2/AC-9/AC-13: Cargo.toml must keep `{suite}` so workspace test --features demo stays green"
        );
    }
    let src = this_suite_source();
    assert!(
        !src.contains("#[ignore"),
    ...
    assert!(
        ...
        "AC-2: target suite must be complete (no todo!/unimplemented!)"
    );
}
...
fn ac3_twenty_six_sdlc_controls_with_domains_evidence_tests() {
    ...
    let assessable: Vec<_> = catalog
        ...
        .keys()
        .filter(|id| {
            SDLC_ID_PREFIXES.iter().any(|p| id.starts_with(p)) && *id != PINNED_FIXTURE_CONTROL
        })
        .collect();
    assert!(
...
fn ac4_evidence_types_are_facts_not_conclusions() {
    ...
    for ev in catalog.evidence().values() {
        if !ev.id.starts_with("evidence.repository.")
            && !ev.id.starts_with("evidence.cicd.")
            && !ev.id.starts_with("evidence.deployment.")
            && !ev.id.starts_with("evidence.release.")
            && !ev.id.starts_with("evidence.supply-chain.")
        {
            continue;
        }
        let used = catalog
    ...
    for id in PROMPT05_EXAMPLE_TESTS {
        ...
        assert!(
            POPULATION_OPS.iter().any(|op| window.contains(op)
                ...
                || window.to_ascii_lowercase().contains("manual-review"),
            "AC-5: {id} must declare a population operator (or honest manual-review)"
        );
    }
...
fn ac6_seven_fixtures_distinguish_evaluator_outcomes() {
    let root = manifest_dir().join("fixtures/assurance/canonical/v1/sdlc");
    assert!(
        root.is_dir(),
    ...
    assert_eq!(
        ok.effectiveness,
        Effectiveness::Effective,
        "AC-6 healthy-population: all default branches protected"
    );
    ...
    assert_eq!(
        ...
        "AC-6 unprotected-repository: fail, got {:?}",
        failed.effectiveness
    );
    ...
    branch_protection(&mut stale, "repo:app", true, 48);
    branch_protection(&mut stale, "repo:lib", true, 48);
    ...
    assert_eq!(
        ...
        "AC-6 stale-evidence: StaleEvidence, not Ineffective-as-missing"
    );

    let (manual, _) = result_json(
        "test.source.security-review",
        "control.source.security-review",
    ...
    assert_eq!(
        ...
        "AC-6 manual-review-required fixture"
    );
    ...
    assert_eq!(
        part.effectiveness,
        Effectiveness::InsufficientEvidence,
        "AC-6: partial/unknown population → InsufficientEvidence, got {:?}",
        part.effectiveness
    );
    ...
    assert_eq!(
        ...
        "AC-6 approved-exception → ExceptionApproved, got {:?} {exc_json}",
        exc.effectiveness
    );
}
...
#[test]
fn ac16_manifest_lists_sdlc_files() {
...
fn ac11_no_second_loader_value_enum_or_repo_inventory_resolver() {
    ...
    assert!(
        !rust.contains("fn resolve_repository_inventory"),
        "AC-11: do not add resolve_repository_inventory; use inventory.subject + inventory.complete"
    );
...
fn ac12_approved_exception_uses_ir_expired_revoked_do_not_pass() {
    ...
    assert_eq!(
        ok.effectiveness,
        Effectiveness::ExceptionApproved,
        "AC-12: approved unexpired Exception IR → ExceptionApproved, got {:?} {json}",
        ok.effectiveness
    );
...
fn ac13_iam_family_and_cat_fixture_ids_remain() {
    ...
    let expr = format!("{:?}", fixture_test.expression);
    assert!(
