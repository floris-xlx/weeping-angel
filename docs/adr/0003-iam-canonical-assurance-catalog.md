...
Canonical catalog infrastructure (Prompt 01), typed evidence (Prompt 02), and subject-population coverage (Prompt 03) landed as sibling ADRs. They provide the loader/validator/digest, fact encoding, and `AllSubjects` / `CoverageAtLeast` / `NoneSubjects` runtime. They do not own identity-domain content.
...
2. What public ID contract do future identity collectors and Prompt 12 ISO remapping consume?
...
Independently assessable IAM controls live in the Prompt 01 tree:
...
Provider details belong only in future collectors that **emit** `evidence.identity.*` facts. Framework details belong only in later mappings (Prompt 12).
...
Fixtures emit these types. No `source.admin.permissions` in IAM fixtures. Credential-shaped keys and compliance narratives remain rejected at seal (Prompt 02).

### 4. Tests are population predicates

`test.identity.privileged-mfa-enabled` means **all in-scope privileged identities have MFA**, using Prompt 03 arms (`coverage-at-least` 100%, `all-subjects`, `none-subjects`). It does not mean “some `mfa-status` envelope exists.”
...
This slice **declares** those tests. It does not reimplement `AllSubjects` / `CoverageAtLeast` (no `IamPopulation` fork). Population resolution already consumes `evidence.identity.inventory` (plus privileged-membership / service-account) in the Prompt 03 runtime.
...
- Prompt 12 can map `iso27001:a.8.5` → `control.identity.privileged-mfa` (and siblings) without rewriting collectors.
...
Entra / Okta / Google Workspace / AD / Cognito collectors; ISO / SOC 2 / NIS 2 / DORA text or mappings; generic rule-engine expansion; PAM / IGA product integrations; certification language; SDLC / vulnerability / infrastructure / governance catalog families (Prompts 05–08).
...
- Spec SSOT: [`docs/sdd/iam-canonical-assurance-catalog.md`](../sdd/iam-canonical-assurance-catalog.md)
- Public contract: [`docs/contracts/assurance-runtime.md`](../contracts/assurance-runtime.md)
