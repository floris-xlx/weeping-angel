# ADR 0035 — Evidence validity and temporal assurance
...
| Field | Value |
| --- | --- |
| Status | **Accepted** — `sdd_evidence_validity_temporal_assurance_target` GREEN; baseline skip-superseded. Product types are the same as [`0018-evidence-validity-temporal-assurance.md`](0018-evidence-validity-temporal-assurance.md). |
| Date | 2026-08-19 |
...
> Filename `0003-*` is shared with catalog-program / operational siblings. Cite this decision by **path**. This file and [`0018-evidence-validity-temporal-assurance.md`](0018-evidence-validity-temporal-assurance.md) record the **same** product decision (one `evidence-validity/v1`, one as-of selector). Dual-suite slugs stay distinct.
...
Same seven clauses as [`0018-evidence-validity-temporal-assurance.md`](0018-evidence-validity-temporal-assurance.md):
...
3. Candidate filter at `T`; `within_window` stays collection-time; `valid_during` / `as_of` (`latest_as_of` alias) / `select_latest_as_of` are the validity APIs. Distinct `current` / `valid_at` are [ADR 0011](0047-temporal-lineage-evidence-soa-integrity.md).
...
- Canonical decision write-up (implemented surfaces): [`0018-evidence-validity-temporal-assurance.md`](0018-evidence-validity-temporal-assurance.md)
