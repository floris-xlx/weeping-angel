# ADR 0029 — Operational Statement of Applicability (graph projection + immutable snapshots)
...
| Extends | [ADR 0001](0001-inwardly-extensible-assurance-runtime.md), [ADR 0002](0002-iso-27001-assurance-vertical.md), [applicability engine](0014-applicability-engine.md), [assessment lineage](0015-assessment-lineage.md), [ISO remap](0027-iso27001-canonical-remap.md) |
...
`src/assurance_soa.rs` dispatches `assurance soa`: banner then JSON. `latest` / empty / named assessment reconstructs from `assurance-ledger.sqlite` via `replay_assessment` + `project_soa_from_snapshot` ([ADR 0011](0047-temporal-lineage-evidence-soa-integrity.md)). Missing ledger, missing run, or replay failure exits non-zero — live `project_soa` is **not** history. No `compilerTopology`. Never emit `ISO 27001 certified` / `compliant` / `certification guaranteed` / `audit passed`.
...
- Historical SoA / four-clock evidence: [ADR 0011](0047-temporal-lineage-evidence-soa-integrity.md)
