...
| Supercedes | The “GitHub evidence types are canonical (`source.branch.protection`, …)” clause of [ADR 0002](0002-iso-27001-assurance-vertical.md) §6 **for newly emitted collector observations**. Does **not** replace ISO GH-007 / GH-009 fail-closed/redaction law, Prompt 12 remap, or the ISO pack as a projection. |
| Extends | [ADR 0001](0001-inwardly-extensible-assurance-runtime.md), [typed evidence](0003-typed-evidence-canonical-serialization.md), [population / coverage](0003-subject-population-runtime-and-coverage-semantics.md), [IAM](0003-iam-canonical-assurance-catalog.md), [SDLC](0003-sdlc-canonical-assurance-catalog.md) |
| Spec | [`docs/sdd/github-collector.md`](../sdd/github-collector.md) |
| Run | [`docs/sdd/sdd-github-collector.md`](../sdd/sdd-github-collector.md) |
...
Prompts 01–05 landed a provider-neutral catalog spine, typed `EvidenceValue`, population completeness (`inventory.subject` / `inventory.complete`), `evidence.identity.*`, and SDLC `evidence.repository.*` / `evidence.cicd.*` / `evidence.deployment.*`. SDLC catalog TOML forbids GitHub tokens in catalog IDs and told that slice not to expand the collector — expansion is this decision (Prompt 09).
...
1. Are newly collected GitHub observations ADR 0002 `source.*` types or Prompt 04/05 `evidence.*` types?
...
Dual-emitting `source.*` and `evidence.repository.*` would make “another provider, same test results” false: ISO existence checks would still pass only for GitHub. Prompt 12 remaps the ISO pack. ISO GH-012 is satisfied by retaining the historical type **strings** in a collector-owned mapping table (`SOURCE_TO_CANONICAL`), not by sealing `source.*` envelopes.
...
An observation `extensions` object and extra `EvidenceProvenance` fields were **not** added (typed-evidence digest law / Prompt 02 ownership). Canonical tests ignore any future provider-native extras. `evidence.github.*` is not emitted and is not required by goldens.
...
Only `crates/weeping-angel-collector/src/github/**`, goldens under `fixtures/assurance/canonical/v1/github/`, `tests/sdd/github_collector.*`, and this program’s SDD/ADR/contract text. No Prompt 06/07/08 catalog TOML rewrite. No ISO pack rewrite.
...
5. **Extend `EvidenceProvenance` with `extensions`** — digest/contract risk owned by Prompt 02; deferred. Traceability uses subject_id + provenance.asset.
6. **Add `failure_behavior` to shared `CollectorDescriptor`** — couples every collector to a Prompt 09 field; rejected.

## Consequences

- `docs/contracts/assurance-runtime.md` GitHub type list is the canonical emit set, not the ADR 0002 `source.*` list.
- ISO pack mappings stay on catalog `control.*` (Prompt 12). GH-012 needles remain mapping-table strings in collector sources.
