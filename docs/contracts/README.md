...
Discovery is the one `[[test]]` harness on package `weeping-angel` (`apps/cli/tests/harness.rs`, and `documentation_layout.rs`). Root `Cargo.toml` is the workspace SSOT, not a suite catalog. Do not maintain a dual-suite inventory here.
...
Layout: [ADR 0004](../adr/0004-documentation-architecture.md). Environment (one harness / virtual workspace): [ADR 0051](../adr/0051-repository-environment.md). Hygiene: [ADR 0012](../adr/0012-repository-hygiene.md), spec [`docs/specs/repository-hygiene.md`](../specs/repository-hygiene.md).
