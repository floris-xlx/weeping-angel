//! Baseline suite for evidence validity and temporal assurance.
//!
//! Characterization of CURRENT behavior on SHA
//! `6e31bf1ae8f4a69227e0557d878f2e76d0cb8f2a` (`docs/specs/temporal-assurance.md`
//! §3). The only envelope clock is `provenance.collected_at`. DigestBody hashes
//! observation+provenance; `supersedes`, `collectionRunId`, and `artifactRef`
//! sit outside the digest. There are no `observed_at`, `valid_from`,
//! `valid_until`, `source_revision`, or revocation envelope fields. Ledger
//! `latest` / `within_window` order and filter `collected_at` only. Population
//! `select_latest` walks `supersedes` then latest `collected_at`+digest with no
//! as-of filter; `Exists` uses digest-order `first_selector` and can conclude
//! `Effective` from one non-stale in-set observation. Stale is
//! `now - collected_at > AssessmentContext.max_age` (or selector freshness);
//! future `collected_at` becomes `StaleEvidence` via `to_std` failure and can
//! shadow an older candidate. Facade `assess()` uses `Utc::now()` and a
//! hard-coded 24h `max_age`. `SnapshotDiff` / `compare` is pairwise readiness,
//! not period coverage. Prompt 13 scheduler is absent.
//!
//! Skip-superseded by `sdd_temporal_assurance_target`
//! (`#[ignore = "superseded by target suite"]`). Does not implement temporal
//! validity.
...
use weeping_angel_assurance_ir::{
    AssessmentId, AssetId, ControlId, RequirementId, canonical_digest,
};
...
fn seal_at(
    ...
    EvidenceEnvelope::seal(
        obs,
        EvidenceProvenance {
            collector_id: "fixture.temporal-baseline".into(),
            collected_at,
            scope: "baseline".into(),
...
fn exists_test() -> CompiledControlTest {
    CompiledControlTest::builder()
        .id(weeping_angel_assurance_ir::ControlTestId::new(
            "test.temporal.baseline.exists",
        ))
        .control_id(ControlId::new("canonical.temporal.baseline"))
...
fn empty_readiness() -> FrameworkReadinessSnapshot {
    FrameworkReadinessSnapshot {
        assessment_id: AssessmentId::new("assess-temporal-baseline"),
...
fn ledger_latest_is_collected_at_desc_without_supersede_walk() {
    ...
    let replacement = older_leaf.clone().with_supersedes(&newer_digest);
    // with_supersedes does not change digest; append is idempotent by digest so
    // build a distinct later-collected envelope that supersedes the newer row.
...
fn snapshot_diff_is_pairwise_readiness_not_period_coverage() {
    ...
    assert_eq!(
        compare_lineage(&run_a, &run_b).framework_pack_digest_changed,
        true
    );
