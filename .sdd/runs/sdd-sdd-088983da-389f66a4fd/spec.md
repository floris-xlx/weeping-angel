...
| Run | `sdd-088983da-389f66a4fd` |
| Snapshot | head `e2def07ee4c3ec265a6b5fee116931f0b2c9ce94` · tree `a6a5eaa1600e4a0e1021598e30ade749aa7ac8da` · commit `ab39c458567270aafc2e3283c28307d0cc551dea` |
...
This document is the durable SSOT for the **SDLC catalog slice** on run `sdd-088983da-389f66a4fd`. It does not replace Prompt 01 catalog infrastructure, Prompt 02 typed evidence, Prompt 03 population runtime, or the Prompt 04 IAM family. Prompts 01–04 have landed on the snapshot head; this slice consumes their loader, `EvidenceValue`, and population evaluator and **must not** invent a second copy.

Tool-state dirty paths excluded from the snapshot (do not treat as product state): `docs/sdd/sdd-sdd-59e9991c-2b5e88f63c/`, `docs/sdd/sdd-sdd-d16b8542-3e47532480/`, `docs/sdd/sdd-sdd-de275e29-f5190941ef/`, `docs/sdd/xylex-sdd-v2-i3-prestate-failure.md`. Admitted dirty semantic paths: the SDLC and vulnerability ADR drafts.
...
Catalog validator already rejects reserved provider/framework **ID segments** (`github`, `gitlab`, `bitbucket`, `iso27001`, `soc2`, …). IDs must be lowercase dotted `kind.family.slug` with `-` allowed; `_` is malformed. `azure-devops` is forbidden in SDLC content even if the validator only lists the `azure` segment.
...
A single `sdlc.toml` per section is acceptable if every id below is present and listed in `[files]`. Do **not** add these controls to `frameworks/iso-27001/2022/metadata.toml`. Do **not** remove or rewrite `fixture.example.toml` / `identity.toml`. Extra files under `catalog/canonical/v1/` that are not listed in the manifest fail `CanonicalCatalog::validate` (`Unlisted`).
...
tree         = a6a5eaa1600e4a0e1021598e30ade749aa7ac8da
commit       = ab39c458567270aafc2e3283c28307d0cc551dea
branch       = main
run          = sdd-088983da-389f66a4fd
