...
| Planning / discovery SHA | `e2def07ee4c3ec265a6b5fee116931f0b2c9ce94` (`main`, 2026-08-19) |
| Dual-suite (register at implement) | `sdd_sdlc_catalog_baseline` · `sdd_sdlc_catalog_target` |
| Transition | **additive** (CI-004: baseline stays GREEN after implement) |
| Mode | `strict` · isolation `worktree` · `max_iters=2` |
...
On SHA `e2def07ee4c3ec265a6b5fee116931f0b2c9ce94` the only SDLC-adjacent product content is:
...
There is no `control.source.default-branch-protection` family, no `evidence.repository.*` / `evidence.cicd.*` / `evidence.deployment.*` / `evidence.release.*` / `evidence.supply-chain.*` contracts (beyond the fixture), and no multi-repository population fixtures. A future GitLab or Bitbucket collector therefore has nowhere canonical to emit SDLC facts.
...
This slice does **not** claim ISO/SOC 2 coverage. Framework remapping is Prompt 12. This slice does **not** expand the GitHub collector (Prompt 09).

---

## 2. Dependencies and fail-closed blockers

| Prompt | Owns | On discovery SHA `e2def07…` | This slice may |
| --- | --- | --- | --- |
...
Inspected: `catalog/canonical/v1/`, `crates/weeping-angel-canonical-catalog`, `weeping-angel-control-test` (`population.rs`), `weeping-angel-collector/src/github/descriptor.rs`, `frameworks/iso-27001/2022/{metadata,mappings}.toml`, `tests/sdd/{iam,canonical,iso27001}_*`, Prompt 05, IAM SSOT.
...
No `controls/source.toml`, `cicd.toml`, `release.toml`, or `supply-chain.toml`. No `evidence.repository.*` family.
...
| evidence | `evidence.source.protected-branch` | envelope type `source.branch.protection` |
| test | `test.source.protected-branch` | `op = "exists"` on that evidence |

IAM family (23 `control.identity.*`) is present and must remain.
...
`GITHUB_EVIDENCE_TYPES` advertises GitHub-native `source.*` names (repository exists/visibility/archived, default branch, branch protection / reviews / status checks / force-push / deletion, CODEOWNERS, admin/collaborator, Dependabot / secret scanning / code scanning, workflow permissions, rulesets, commit signing). Subject types: `repository`, `branch`.

Catalog tests must **not** import or assert against `GITHUB_EVIDENCE_TYPES`. Scanner engines (`src/engines/*`, `src/depcheck/*`) are not evidence contracts.

### 3.4 Population runtime (consume, do not change)

`resolve_population` order:

1. explicit `EvidenceSet` population;
2. identity inventory (`evidence.identity.inventory` + privileged / service-account);
3. generic `inventory.subject` + `inventory.complete` (`authoritative`);
4. otherwise infer subject ids from the observation type with **Unknown** completeness.
...
There is **no** repository-inventory special case. This slice must not add one.

### 3.5 What “SDLC assessment” means today
...
- evaluate force-push, deletion, reviewer count, CODEOWNERS, workflow write permissions, protected environments, provenance, or lockfile integrity as population predicates;
- accept GitLab/Bitbucket-shaped facts without teaching tests those providers.

The baseline suite therefore characterizes **invariants that remain true after this slice** (ISO sliver, fixture exists-check, IAM family, collector not remapped), not “absence of every `control.source.*` string” (that would break on the fixture and would fail after additive implement).
...
Each control record must carry: stable id, title, description/objective, domain(s) from existing `ControlDomain` (`SecureDevelopment`, `ChangeManagement`, `VulnerabilityManagement`, `Governance` as appropriate), evidence-requirement refs, test refs, and an honest automation class (`Automated` | `Hybrid` | `Manual`).
...
Envelope `evidence_type` in fixtures should equal the **catalog evidence id** (same convention as IAM fixtures). Collectors later normalize provider payloads into these types.
...
- Typed facts: Prompt 02 `EvidenceValue`. Prefer `with_value`; `with_fact` remains string-compat.
...
Follow the existing root `[[test]]` pattern. **Additive:** baseline encodes invariants that remain true after implement (CI-004). Do **not** write baseline as “no `control.source.default-branch-protection` exists” — that would be replacement and would fail post-implement.
...
| Target | `tests/sdd/sdlc_catalog.target.rs` · `sdd_sdlc_catalog_target` | RED until SDLC content lands; then **GREEN** — CI gate (SDLC-001…016). |
...
1. **AC-1.** Dual-suite `sdd_sdlc_catalog_baseline` + `sdd_sdlc_catalog_target` is registered in root `Cargo.toml` like existing SDD tests; paths `tests/sdd/sdlc_catalog.baseline.rs` and `tests/sdd/sdlc_catalog.target.rs`.
...
- Vulnerability / infrastructure / governance catalog families (Prompts 06–08) beyond the SDLC-adjacent dependency-scan / unsupported-component controls listed here.
...
| GitHub collector “helpfully” emits new types | Out of scope; AC-10. |
| Catalog tests import `GITHUB_EVIDENCE_TYPES` | SDLC-014. |
...
Draft: [`docs/adr/0003-sdlc-canonical-assurance-catalog-draft.md`](../../adr/0003-sdlc-canonical-assurance-catalog-draft.md).
...
```text
planning_sha = e2def07ee4c3ec265a6b5fee116931f0b2c9ce94
branch       = main
note         = prompts 01–04 landed (catalog fixture + IAM family + typed evidence +
               ...
               ISO source sliver + exists-only fixture only
```
...
**Live seams:** catalog TOML + manifest listing; Prompt 03 population completeness; ISO pack compile; GitHub collector advertisement (read-only); IAM and CAT dual-suites.

**Tooling:** language Rust; package manager Cargo (pnpm only for `apps/docs`); test framework `cargo test`.
